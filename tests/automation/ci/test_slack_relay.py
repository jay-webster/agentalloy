...
from automation.ci import slack_relay


def test_main_posts_report_from_env_to_slack(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    posted = []
    monkeypatch.setattr(
        slack_relay, "post_to_slack", lambda message, webhook_url: posted.append(message)
    )

    exit_code = slack_relay.main()
...
def test_main_empty_webhook_url_skips_gracefully(
    ...
    monkeypatch.setattr(slack_relay, "post_to_slack", _fail_if_called)
