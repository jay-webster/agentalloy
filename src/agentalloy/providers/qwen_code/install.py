...
activation carriers and a ``.qwen/.agentalloy-env`` file. The implementation
lives in ``wire_harness._wire_proxy_qwen_code`` (shared with ``agentalloy
wire``); this module delegates so the provider registry and the live wiring
can never diverge.
"""
...
def extract_upstream(root: Path) -> Upstream | None:
    ...
    with a ``/proj/<token>`` path, so we skip if the baseUrl contains
    ``/proj/`` (avoiding adopting the proxy as its own upstream).
...
def apply_persistent_config(port: int, root: Path, force: bool = False) -> list[WireRecord]:
    ...
    Args:
        ...
        force: Unused — qwen-code wiring replaces only files it owns.
