...
def _extract_task_from_messages(request: ProxyRequest) -> str | None:
    ...
    signal-keyword trigger to that opening line for the rest of the session,
    silently disabling keyword-driven phase transitions. Scan in reverse
    instead, so this always reflects the current turn.
...
def _evaluate_free_flow(
    ...
    *,
    ...
    Carrier-gated like workflow mode: only identifiable sessions (``session_key``
    present) trigger compose/cadence; anonymous requests are forwarded silently.
    """
    task = _extract_task_from_messages(request)
    repo = str(cwd)
    session_key, session_source = resolve_session_key(request, session_id)

    # Same carrier rule as workflow mode (see the gate in evaluate_signal): only
    # identifiable sessions carry.
    if not session_key:
        return SignalResult(
...
async def evaluate_signal(
    ...
    *,
    ...
    # Who set *phase* as of the start of this turn — never this turn's own write
    # (the auto-advance below happens later, in step 6). Used to detect a "swept"
    # phase: this session is about to act on a phase a DIFFERENT concrete session
    # put in place since this session last looked (see `_boundary_confirm_directives`).
    ...
    #
    # Every supported harness (Claude Code, aider, etc.) is either header-keyed or
    # fingerprint-keyed, so ``session_key`` is the sole signal: if we have a
    # session id, we're an identifiable agent session and compose/inject cadence
    # apply. Anonymous requests (no session header, no user message for
    # fingerprinting) are forwarded silently — they don't carry enough identity
    # to justify burning the once-per-session orientation markers.
    #
    # A prior version carved out tool-less HEADER-source requests as non-carriers
    # (to dodge a race where a tool-less background ping burns the Tier 1/Tier 2
    # markers before the real conversation gets them), while always treating
    # FINGERPRINT-source sessions as carriers. That discriminated between session
    # sources: header-sourced sessions were only carriers if they had tools,
    # while fingerprint-sourced sessions always were — starving header-sourced,
    # tool-less harnesses of compose/inject on their first turn. The per-session
    # announcement/confirmation logic already ensures orientation and cursors are
    # burned exactly once per (phase, session), so there's no race between
    # main-loop and background requests to guard against.
...
def _boundary_confirm_directives(
    ...
    *,
    ...
    single coherent directive list — at most one prompt, never conflicting MUST
    blocks:
    ...
    - **T3 swept by another session** — an *already-oriented* session (not
      ``new_session``) observes the phase changed since it last looked
      (``phase_changed``), AND the recorded ``transitioned_by`` actor is a
      concrete, *different* session key from this turn's ``session_key``. This
      is distinct from T2: T2 covers a session's first turn on a phase it never
      saw change; T3 covers a session that WAS oriented, had the phase moved out
      from under it by someone else, and needs to be told before charging ahead
      on the new phase as if it had caused the transition itself. Mutually
      exclusive with ``new_session`` by construction (see the call site: a
      brand-new session can't have "just been swept" — it never had a prior
      orientation to be swept from). ``transitioned_by`` or ``session_key``
      unset (bare CLI invocation, ambiguous actor) never triggers T3 — silence
      on an unattributable transition beats a false positive.

    Precedence when multiple apply: a single combined prompt per case — e.g. a
    new session landing on ``ship`` with a delivery record gets one combined T1+T2
    prompt (confirm the phase, then ask about the reset); a swept session landing
    on ``ship`` similarly gets one combined T1+T3 prompt. T2 and T3 cannot both
    apply (mutually exclusive by construction above).
    """
