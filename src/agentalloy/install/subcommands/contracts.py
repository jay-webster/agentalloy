...
the singular ``contract`` group (validate/show/init a single file). Ships:
    ...
    agentalloy contracts archive [--phase P] [--slug S] [--dry-run]
...
``archive`` moves live contracts from ``active/<phase>/`` to
``archive/<phase>/``, the same sweep ``run_phase_set`` performs automatically
on a ship→intake reset — exposed here for a manual mid-cycle archive.
"""
